...
class PostController extends Controller
{
     ...
     */
    public function unvotePoll(Request $request, Post $post)
    {
        \Log::info('Unvote attempt', [
            'post_id' => $post->id,
            'user_id' => auth()->id(),
        ...
        \Log::info('Vote removed successfully', ['poll_id' => $poll->id, 'option_id' => $optionId]);

        // Reload poll with options
        $poll->load('options');

        // Check if user still has other votes
        $userHasVoted = PollVote::where('poll_id', $poll->id)
            ->where('user_id', $user->id)
            ->exists();

        // Get user's voted option IDs
        $userVotedOptions = PollVote::where('poll_id', $poll->id)
            ->where('user_id', $user->id)
    ...
    public function votePoll(Request $request, Post $post)
    {
        ...
        if ($post->type !== 'poll' || !$post->poll) {
            \Log::error('Not a poll', ['post_id' => $post->id, 'type' => $post->type]);
            return response()->json([
                'success' => false,
                'message' => 'This post is not a poll'
            ], 400);
        ...
        if (now()->greaterThan($poll->expires_at)) {
            \Log::warning('Poll expired', ['poll_id' => $poll->id, 'expires_at' => $poll->expires_at]);
            return response()->json([
                'success' => false,
        ...
        if (!$option) {
            \Log::error('Invalid option', ['option_id' => $optionId, 'poll_id' => $poll->id]);
            return response()->json([
                'success' => false,
        ...
        if ($existingVote) {
            ...
            if (!$poll->allow_multiple) {
                \Log::info('Already voted (single choice)', ['poll_id' => $poll->id, 'user_id' => $user->id]);
                return response()->json([
                    'success' => false,
            ...
            if ($alreadyVotedForOption) {
                \Log::info('Already voted for this option', ['poll_id' => $poll->id, 'option_id' => $optionId]);
                return response()->json([
                    'success' => false,
        ...
        \Log::info('Vote recorded successfully', ['poll_id' => $poll->id, 'option_id' => $optionId, 'user_id' => $user->id]);
    ...
    private function formatPost($post, $user)
    {
        ...
        if ($post->type === 'poll' && $post->poll) {
            ...
            // Get user's voted option IDs
            $userVotedOptions = PollVote::where('poll_id', $post->poll->id)
                ->where('user_id', $user->id)
                ->pluck('poll_option_id')
                ->toArray();
